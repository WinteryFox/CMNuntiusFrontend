...
export default function Chat(props: {
    ...
    async function sendMessage(event: React.MouseEvent) {
        ...
        if (!props.history || !input)
            return

        setSending(() => true)
        try {
            const response = (await rest.post("/messages", {
                sender: props.history[0].to.number,
                recipient: props.history[0].from.number,
                content: input,
