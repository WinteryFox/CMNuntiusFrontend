...
export default function Chat(props: {
    ...
    const chatEnd = React.createRef<HTMLDivElement>()
    const [input, setInput] = useState<string>("")
    const [isSending, setSending] = useState<boolean>(false)

    function onKeyDown(event: React.KeyboardEvent<HTMLDivElement>) {
        if (event.key == "Enter" && !event.shiftKey) {
            sendMessage(event.currentTarget.innerText).then()
        }
    }
    ...
    return (
        <div className={"flex flex-col bg-white w-full"}>
            <div className="flex items-center justify-between border-b py-3 px-10">
            ...
            <div className="flex px-10 pb-5 bg-gray-50">
                <ContentEditable className={"w-full bg-white rounded-3xl px-5 py-2"}
                                 html={input} onChange={(event) => setInput(() => event.target.value)}
                                 onKeyDown={(event) => onKeyDown(event)} disabled={isSending}/>
            </div>
        </div>
    );
}
